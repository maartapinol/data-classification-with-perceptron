...
def PerceptronTraining(TrainData, labelVar, class1, class2, MaxNumIter, l2):
    ...
    for iter in list(range(MaxNumIter)):
        ...
        for i in list(range(len(TrainDataX))):
            ...
            if float(TrainDataY[i]*a) <= 0: 
                weights = weights + l2*(TrainDataY[i] - np.sign(a))*TrainDataX.iloc[i] # update weights
                bias = bias + TrainDataY[i] # update bias
    
    fx = []
    for i in list(range(len(TrainDataX))):
        fx = fx + [sum(weights*TrainDataX.iloc[i]) + bias]

    sign = np.sign(fx) # compute sign of f(x)
    accuracy = sum(TrainDataY == sign)/(sum(TrainDataY == sign) + sum(TrainDataY != sign)) # compute accuracies
...
y = "y"
class1 = ["class-1"]
class2 = ["class-2","class-3"]

w,b,train_accuracy = PerceptronTraining(traindf, y, class1, class2, Iter, l2)
